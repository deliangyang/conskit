...
void resolve_task_annotations(zend_string *task_class_name, zval *annoations) {
    ...
    zend_hash_add(Z_ARRVAL_P(annoations), zend_string_init(ZEND_STRL("queues"), 0), &queues);
	zend_hash_add(Z_ARRVAL_P(annoations), zend_string_init(ZEND_STRL("exchanges"), 0), &exchanges);
	zend_hash_add(Z_ARRVAL_P(annoations), zend_string_init(ZEND_STRL("bind"), 0), &bind_queues);
}


void execute(zval *message) {

    zval *z = zend_hash_find(Z_ARRVAL_P(message), zend_string_init(TASK_MESSAGE_CLS));
    char *s = Z_STRVAL_P(z);

    zval *zid = zend_hash_find(Z_ARRVAL_P(message), zend_string_init(TASK_MESSAGE_ID));
    char *id = Z_STRVAL_P(zid);
    
    zval *args = zend_hash_find(Z_ARRVAL_P(message), zend_string_init(TASK_MESSAGE_ARGS));
       
    zend_string *pcls_name = zend_string_init(ZEND_STRL("Haystack\\Task\\Parameter"), 0);
